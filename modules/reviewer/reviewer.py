...
from modules.reviewer.auxiliary import get_accuracy, get_win_difference
...
class Reviewer(Processor):
    def review_game(
        ...
        for idx, move in enumerate(moves):
            ...
            move_classification = self.review_move(board.turn, move, evaluation, best_moves, evaluations, review.moves)
            ...
            if best_evaluation.mate:
                if best_evaluation.value > 0:
                    if evaluation.mate:
                        if evaluation.value >= 0:
                            only_one_way_to_mate = (
                                ONE_WAY_TO_MATE_DISTANCE_MIN <= best_evaluation.value <= ONE_WAY_TO_MATE_DISTANCE_MAX
                                and not (evaluations[1].mate and evaluations[1].value > 0)
                            )
                ...
                if move in best_moves:
                    ...
                    if win_difference > BLUNDER_THRESHOLD and evaluation.value < BLUNDER_PAWN_THRESHOLD:
                        ...
                        move_type = (
                            "miss"
                            if history and history[-1].move_classification.type in ["inaccuracy", "mistake", "blunder"]
                            else "mistake"
                        )
        ...
        for best_move, move_evaluation in zip(best_moves, evaluations):
            if any(
                [
                    ...
                    not best_evaluation.mate
                    and not move_evaluation.mate
                    and move_evaluation.value >= best_evaluation.value - BEST_MOVE_PAWN_TOLERANCE,
                ]
            ):
